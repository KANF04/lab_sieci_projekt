...
// UWAGA TEN PLIK SIE NAJBARDZIEJ ZMIENI -- poki co zostawiam bo nie wiem jak jeszcze rozwiazemy problem z wysylaniem danych
...
/*Funkcja ctrl_c ma na celu obsłużenie sygnału przerwania (Ctrl+C) i zamknięcie serwera.
...
/*Funkcja sendToAllBut ma na celu wysłanie wiadomości do wszystkich klientów oprócz nadawcy.

args:
    - **int** fd - deskryptor klienta, który ma zostać pominięty
    - **const char** *buffer - bufor z danymi
    - **int** count - liczba bajtów do wysłania
*/
void sendToAllBut(int fd, const char *buffer, int count) {
